const UA = "Multi-Model-AI/1.0 (autonomous media asset search)";

function cleanQuery(value) {
  return String(value || "")
    .replace(/\b(4k|8k|hd|1080p|vertical|stock footage)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 180);
}

function scoreResult(title, query, mime, size) {
  ...
  if (/video\//i.test(mime || "") || /webm|mp4|mov|ogv/i.test(mime || "")) score += 0.15;
...
async function searchCommons(query, limit = 6) {
  const params = new URLSearchParams({
    action: "query",
    generator: "search",
    gsrsearch: `filetype:video ${cleanQuery(query)}`,
    gsrnamespace: "6",
  ...
  return pages.map(page => {
    ...
    return {
      ...
      score: scoreResult(title, query, info.mime, info.size)
    };
...
async function searchOne(item) {
  ...
  if (!query) return { ...item, assets: [], status: "no_query" };

  try {
    const assets = await searchCommons(query, 6);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!items.length) {
      return res.status(400).json({ error: "broll array is required." });
    }

    const results = [];
    for (const item of items) {
      results.push(await searchOne(item));
    }
    ...
    if (attach && plan) {
      ...
      for (let i = 0; i < Math.min(plan.broll.length, results.length); i++) {
        ...
        plan.broll[i] = {
          ...
          asset_status: "candidate_found",
          asset_source: picked.source,
          asset_title: picked.title,
          asset_page_url: picked.page_url,
          asset_license: picked.license,
          asset_creator: picked.creator,
          asset_match_score: picked.score
        };
      }
    }
    ...
    return res.status(200).json({
      ...
      license_note: "Each result includes the license metadata returned by the source. Verify attribution/usage requirements before publishing.",
      results,
      plan
    });
  } catch (error) {
    return res.status(500).json({
      error: error.message || "Asset intelligence failed."
    });
  }
}
